pipeline {
    agent any
    environment {
        image="535324349245.dkr.ecr.ap-northeast-1.amazonaws.com/bb-order:"
    }
    stages {
        stage('获取CommitID') {
            steps {
        ...
        stage('发布') {
            steps {
                sh """
                    kubectl --kubeconfig /home/jenkins/.kube/ex-pro -nex-pro set image deployment bb-order bb-order=${env.image}${imageTag}-${env.BUILD_ID}
                """
            }
        }
    }
